...
if [ "$color_prompt" = yes ]; then
    PROMPT=$'%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)──}(%B%F{%(#.red.blue)}%n%(#.💀.㉿)%m%b%F{%(#.blue.green)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}]-[%F{blue}IP:%F{green}][%F{reset}$(whatvpn)%F{green}]-[%F{blue}Target:%F{green}][%F{reset}\$(/bin/cat /tmp/target.txt)%F{green}]\n└─%B%(#.%F{red}#.%F{blue}$)%b%F{reset} '
